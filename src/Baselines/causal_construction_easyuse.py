...
class CausaltyGraph4Visit:
    def __init__(self, data_all, data_train, num_diagnosis, num_procedure, num_medication, dataset):
        ...
        self.diag_index_map = {f"Diag_{i}": i for i in range(self.diag_med_effect.to_numpy().shape[0])}
        self.proc_index_map = {f"Proc_{i}": i for i in range(self.proc_med_effect.to_numpy().shape[0])}
        self.med_index_map = {f"Med_{i}": i for i in range(self.diag_med_effect.to_numpy().shape[1])}
        self.diag_med_effect_np = self.diag_med_effect.to_numpy()
        self.proc_med_effect_np = self.proc_med_effect.to_numpy()
    # 返回就诊中的一个子图
    ...
    def get_effect(self, a, b, A_type, B_type):
        ...
        if A_type == "Diag" and B_type == "Med":
            a_idx = self.diag_index_map[f"{A_type}_{int(a)}"]
            b_idx = self.med_index_map[f"{B_type}_{int(b)}"]
            return self.diag_med_effect.iloc[a_idx, b_idx]
        elif A_type == "Proc" and B_type == "Med":
            a_idx = self.proc_index_map[f"{A_type}_{int(a)}"]
            b_idx = self.med_index_map[f"{B_type}_{int(b)}"]
            return self.proc_med_effect.iloc[a_idx, b_idx]
        else:
            raise ValueError("Invalid A_type and B_type combination")
        
    def get_effect_batch(self, sources, targets, src_type, tgt_type):
        if src_type == "Diag" and tgt_type == "Med":
            source_indices = [self.diag_index_map[f"{src_type}_{int(s)}"] for s in sources]
            target_indices = [self.med_index_map[f"{tgt_type}_{int(t)}"] for t in targets]
            return self.diag_med_effect_np[np.ix_(source_indices, target_indices)]
        elif src_type == "Proc" and tgt_type == "Med":
            source_indices = [self.proc_index_map[f"{src_type}_{int(s)}"] for s in sources]
            target_indices = [self.med_index_map[f"{tgt_type}_{int(t)}"] for t in targets]
            return self.proc_med_effect_np[np.ix_(source_indices, target_indices)]
        else:
            raise ValueError("Invalid src_type and tgt_type combination")
    ...
    def build_effect(self, num_a, num_b, a_type, b_type):
        ...
        file_path =  f"/home/chenjian/mm_EHR/drugrec/RAREMed/data/output/{self.dataset}/graphs/{a_type}_{b_type}_causal_effect.pkl"
        # 构建完整的文件路径
        try:
    ...
    def build_graph(self, data_all):
        current_dir = os.path.dirname(os.path.abspath(__file__))
        # 构建完整的文件路径
        file_path = f'/home/chenjian/mm_EHR/drugrec/RAREMed/data/output/{self.dataset}/graphs/causal_graph.pkl'
        try:
    ...
    def data_process(self, data_train):
        ...
        current_dir = os.path.dirname(os.path.abspath(__file__))
        # 构建完整的文件路径
        file_path = f'/home/chenjian/mm_EHR/drugrec/RAREMed/data/output/{self.dataset}/graphs/matrix4causalgraph.pkl'
        try:
