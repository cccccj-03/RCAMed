...
from utils.util import multi_label_metric, ddi_rate_score, get_n_params, create_log_id, logging_config, get_grouped_metrics, \
    get_model_path, get_pretrained_model_path,  buildPrjSmiles
from Baselines.causal_construction_easyuse import CausaltyGraph4Visit
from sklearn.metrics import f1_score
import torch.nn as nn
from helper import relevance_mining, med_rel

# Training settings
def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('-n', '--note', type=str, default='', help="User notes")
    parser.add_argument('--model_name', type=str, default='RAREMed_graph', help="model name")
    ...
    parser.add_argument('--weight_multi', type=float, default=0.03, help='weight of multilabel_margin_loss')        # 严重影响性能。增大weight_multi会提高药物数量（提高正样本预测值），进而提高ddi rate。增大multi会增大bce loss。性能先上升后下降，0.01以后明显影响性能
    parser.add_argument('--weight_ddi', type=float, default=0.7, help='weight of ddi loss')         # weight_ddi 越大，loss越高，推荐的药物越少。性能先上升后下降。0.5以上开始明显影响jaccard
...
# evaluate
@torch.no_grad()
def evaluator(args, model, data_val, drug_data, voc_size, epoch, 
    ...
    for patient in tqdm(data_val, ncols=60, total=len(data_val), desc=f"Evaluation"):
        ...
        results, loss_ddi = model(patient, **drug_data)
...
@torch.no_grad()
def evaluator_mask(model, data_val, drug_data, voc_size, epoch, device, mode='pretrain'):
    model.eval()
    loss_val = 0
    ...
    for batch in tqdm(data_val, ncols=60, desc=mode, total=len_val):
        ...
        result, result_med = model(batch, mode=mode, **drug_data)
...
@torch.no_grad()
def evaluator_nsp(model, data_val, data, drug_data, epoch, device, mode='pretrain_nsp'):
    model.eval()
    loss_val = 0
    prc = []
    for batch_data in tqdm(data_val, ncols=60, desc=mode, total=len(data_val)):
        ...
        result, _, _ = model(nsp_batch, mode='pretrain_nsp', **drug_data)
        ...
        f1_macro = f1_score(nsp_target, predictions, average='macro')
        prc.append(np.mean(f1_macro))
    ...
    for visit in batch_data:
        ...
        for i in range(neg_sample_rate):
            ...
            if random.random() < 0.5:
                ...
                nsp_target.append(2)
            nsp_batch.append([neg_visit[0], neg_visit[1]])
            nsp_target.append(3)
...
def main(args):
    ...
    substruct_smiles_list = dill.load(open(substruct_smile_path, 'rb'))
    ddi_mask_H = torch.from_numpy(dill.load(open(ddi_mask_path, 'rb')))#.to(device)
    ...
    molecule_para = {
        'num_layer': 2, 'emb_dim': 128, 'graph_pooling': 'mean',
        'drop_ratio': args.dropout, 'gnn_type': 'gin', 'virtual_node': False
    }
    ...
    ehr_adj = relevance_mining(args, data_train, voc_size)
    med_adj = med_rel(args, data_train, voc_size)

    disease_frequencies = np.sum(ehr_adj[:voc_size[0], :], axis=1)
    procedure_frequencies = np.sum(ehr_adj[voc_size[0]:, :], axis=1)
    # 2. 获取最低10%的频率
    disease_threshold = np.percentile(disease_frequencies, 25)
    procedure_threshold = np.percentile(procedure_frequencies, 25)
    # 3. 筛选出频率低于最低10%的疾病和治疗代码
    rare_diseases = np.where(disease_frequencies <= disease_threshold)[0].tolist()
    rare_procedure = np.where(procedure_frequencies <= procedure_threshold)[0].tolist()
    ...
    model = RAREMed(args, rare_diseases, rare_procedure, causal_graph, voc_size, ddi_adj, molecule_para, substruct_para, substruct_num=ddi_mask_H.shape[1])
    ...
    if args.test:
        ...
        evaluator(args, model, data_test,  drug_data, voc_size, 0, ddi_adj_path, device, rec_results_path)
    ...
    optimizer = Optimizer(model.parameters(), lr=args.lr, weight_decay=args.weight_decay, betas=(0.9, 0.994))  #0.995
    logging.info(f'Optimizer: {optimizer}')

    if args.dataset == 'mimic-iii':
        if args.pretrain_nsp:
            main_nsp(args, model, optimizer, writer, data_pretrain, data_train, data_val, drug_data, device, save_dir, log_save_id)
        if args.pretrain_mask:
            main_mask(args, med_adj, ehr_adj, model, optimizer, writer, diag_voc, pro_voc, data_train, data_val, drug_data,
        ...
        if args.pretrain_nsp:
            main_nsp(args, model, optimizer, writer, data_pretrain, data_train, data_val, drug_data, device, save_dir, log_save_id)
        if args.pretrain_mask:
            main_mask(args, model, optimizer, writer, diag_voc, pro_voc, data_train, data_val, drug_data,
                    voc_size, device, save_dir, log_save_id)
    ...
    # optimizer = Optimizer(model.parameters(), lr=args.lr, weight_decay=0.1)    
    EPOCH = 60
    ...
    for epoch in range(EPOCH):
        ...
        for step, patient in tqdm(enumerate(data_train), ncols=60, desc="finetune", total=len(data_train)):  # every patient
            result, loss_ddi = model(patient, **drug_data)   
            loss_uni = uniformity(model.rare_emb_d.weight.data) + uniformity(model.rare_emb_p.weight.data) #原本为0
            loss_bce, loss_multi = loss_func(voc_size, patient, result, device)
            loss_all = (1 - args.weight_multi) * loss_bce + args.weight_multi * (loss_multi +loss_uni) + args.weight_ddi * loss_ddi 
        ...
        loss_val_bce, loss_val_multi, loss_val_ddi, loss_val_all = evaluator(
            args, model, data_val, drug_data, voc_size, epoch, ddi_adj_path, device)
...
def uniformity(x):
    ...
    x = F.normalize(x, dim=-1)  # [14, 512]
    dist_matrix = torch.cdist(x, x, p=2).pow(2)  # [14, 14]
    loss = dist_matrix.exp().mean().log()  # 求平均后取对数
    return loss + ((1 - x) ** 2).mean()


class OnlineKMeans:
    def __init__(self, n_clusters: int, n_features: int, lr: float = 0.01, device='cuda'):
        self.n_clusters = n_clusters
        self.n_features = n_features
        self.lr = lr
        self.centroids = torch.randn(n_clusters, n_features).to(device)  # 随机初始化质心

    def predict(self, x: torch.Tensor, index) -> torch.Tensor:
        centroids = self.centroids[:, index]
        distances = torch.cdist(x, centroids)
        return torch.argmin(distances, dim=1)

    def update(self, x: torch.Tensor, index):
        cluster_indices = self.predict(x, index)
        centroids = self.centroids[:, index]
        for i in range(self.n_clusters):
            mask = cluster_indices == i
            if torch.any(mask):
                cluster_mean = x[mask].mean(dim=0)
                centroids[i] = (1 - self.lr) * centroids[i] + self.lr * cluster_mean
                self.centroids[:, index] = centroids

def main_mask(args, med_adj, ehr_adj, model, optimizer, writer, diag_voc, pro_voc, data_train, data_val, drug_data, voc_size, device, save_dir, log_save_id):
    epoch_mask = 0
    best_epoch_mask, best_ja_mask = 0, 0
    EPOCH = args.pretrain_epochs
    diag_indices = torch.arange(voc_size[0]).to(device)
    proc_indices = torch.arange(voc_size[1]).to(device)
    kmeans_model = OnlineKMeans(n_clusters=2, n_features=med_adj.shape[1], lr=1e-4, device=device)

    total_frequencies = ehr_adj.sum(axis=1)  # Sum across rows
    disease_frequencies = total_frequencies[:voc_size[0]]  # First i entries
    procedure_frequencies = total_frequencies[voc_size[0]:]  # Last j entries
    ...
    for epoch in range(EPOCH):
        ...
        for batch in tqdm(data_train, ncols=60, desc="pretrain_mask", total=len(data_train)):
            ...
            if args.mask_prob > 0:
                ...
                masked_batch = batch
                masked_data = []
                for visit in masked_batch:
                    diag = visit[0]
                    pro = visit[1]
                    threshold_d = np.percentile(disease_frequencies[diag], 25 + 25 * epoch/EPOCH)  # Example: 100 - epoch
                    threshold_p = np.percentile(procedure_frequencies[pro], 25 + 25 * epoch/EPOCH)  # Example: 100 - epoch
                    disease_mask = disease_frequencies >= threshold_d
                    procedure_mask = procedure_frequencies >= threshold_p

                    diag1 = [diag[i] for i in range(len(diag)) if disease_mask[diag[i]]]
                    pro1 = [pro[i] for i in range(len(pro)) if procedure_mask[pro[i]]]
                    masked_data.append([diag1, pro1])

                    disease_mask = disease_frequencies <= threshold_d
                    procedure_mask = procedure_frequencies <= threshold_p
                    diag2 = [diag[i] for i in range(len(diag)) if disease_mask[diag[i]]]
                    pro2 = [pro[i] for i in range(len(pro)) if procedure_mask[pro[i]]]
                    masked_data.append([diag2, pro2])

            result, result_med = model(masked_data, mode='pretrain_mask', **drug_data)
            bce_target_dis = np.zeros((batch_size*2, voc_size[0]))
            bce_target_pro = np.zeros((batch_size*2, voc_size[1]))
            
            for i in range(0, batch_size, 2):
                bce_target_dis[i, batch[i][0]] = 1
                bce_target_pro[i, batch[i][1]] = 1
                bce_target_dis[i+1, batch[i][0]] = 1
                bce_target_pro[i+1, batch[i][1]] = 1                
            ...
            for adm in batch:
                ...
                index = diseases + [(voc_size[0] + i) for i in procedures]
                
                med_label = torch.tensor(med_adj[:, index], device=device).float()
                kmeans_model.update(med_label, index)
                med_label = kmeans_model.predict(med_label, index)
                # med_label = med_adj[:, index]
                # mean_value = med_adj.mean(axis=1).mean()
                # med_label = (med_adj.mean(axis=1) > mean_value).astype(int)

                # # 构造标签，大于均值的为1，小于均值为0
                med_target.append(torch.tensor(med_label, device=device).float())
                med_target.append(torch.tensor(med_label, device=device).float())
                # med_target.append(torch.tensor(med_label, device=device).float())


                diseases = torch.LongTensor(diseases).to(device)
                procedures = adm[1]
                procedures = torch.LongTensor(procedures).to(device)
                diag_mask = ~torch.isin(diag_indices, diseases)
                diag_indices = diag_indices[diag_mask]
                proc_mask = ~torch.isin(proc_indices, procedures)
                proc_indices = proc_indices[proc_mask]                
                disease_embedding = model.embeddings[0](diag_indices) # (n, 1, dim)
                procedure_embedding = model.embeddings[1](proc_indices)  # (m, 1, dim)

                loss_uni = loss_uni + uniformity(disease_embedding) + uniformity(procedure_embedding)
            med_target = torch.stack(med_target, dim=0)

            loss_med = F.binary_cross_entropy_with_logits(result_med, med_target)
            loss_bce = F.binary_cross_entropy_with_logits(result, torch.tensor(bce_target, device=device))
            loss =  loss_bce + 0.05 * loss_med + 0.5 * loss_uni  # 0.1
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
        ...
        # validation
        loss_val, ja, prauc, avg_p, avg_r, avg_f1, avg_cnt = evaluator_mask(model, data_val, drug_data, voc_size, epoch, device, mode='pretrain_mask')
...
def main_nsp(args, model, optimizer, writer, data, data_train, data_val, drug_data, device, save_dir, log_save_id):
    ...
    for epoch in range(EPOCH):
        ...
        for batch in tqdm(data_train, ncols=60, desc="pretrain_nsp", total=len(data_train)):  # every patient
            ...
            pairs = torch.FloatTensor([True for item in batch])
            result, repr_d, repr_p = model(nsp_batch, mode='pretrain_nsp', **drug_data)
            mask = torch.ones_like(pairs).to(device)
            if nsp_target[1] == 1:
                loss_sim = (alignment_cos_sim(repr_p[0:1], repr_p[1:2]).abs() * mask) / max(mask.sum(), 1e-6) + 0.5 * alignment(repr_d[0:1], repr_d[1:2])#clip_loss(repr_d[0:1], repr_d[1:2], 0.07, device)
            elif nsp_target[1] == 2:
                loss_sim = (alignment_cos_sim(repr_d[0:1], repr_d[1:2]).abs() * mask) / max(mask.sum(), 1e-6) + 0.5 * alignment(repr_p[0:1], repr_p[1:2]) # clip_loss(repr_p[0:1], repr_p[1:2], 0.07, device)
            loss_neg = (alignment_cos_sim(repr_p[0:1], repr_p[2:]).abs() * mask) / max(mask.sum(), 1e-6) + \
                (alignment_cos_sim(repr_d[0:1], repr_d[2:]).abs() * mask) / max(mask.sum(), 1e-6)
            loss_sim = loss_sim + loss_neg / 2
            loss_uni = uniformity(model.rare_emb_d.weight.data) + uniformity(model.rare_emb_p.weight.data) #原本为0
            loss = F.cross_entropy(result, torch.tensor(nsp_target, device=device)) \
                  + loss_sim + 0.5 * loss_uni # , dtype=torch.float32))
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
            loss_train += loss.item()
        loss_train /= len(data_train)
        # validation
        precision, loss_val = evaluator_nsp(model, data_val, data, drug_data, epoch, device, mode='pretrain_nsp')
