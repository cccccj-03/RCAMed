...
from models.ours import RCAMed
...
from helper import relevance_mining, med_rel, count_code_occurrences
...
def get_args():
    ...
    parser.add_argument('--model_name', type=str, default='RCAMed2', help="model name")
    ...
    parser.add_argument('--weight_multi', type=float, default=0.03, help='weight of multilabel_margin_loss')        # 严重影响性能。增大weight_multi会提高药物数量（提高正样本预测值），进而提高ddi rate。增大multi会增大bce loss。性能先上升后下降，0.01以后明显影响性能
    parser.add_argument('--weight_ddi', type=float, default=0.75, help='weight of ddi loss')         # weight_ddi 越大，loss越高，推荐的药物越少。性能先上升后下降。0.5以上开始明显影响jaccard
...
def main(args):
    ...
    ehr_adj = count_code_occurrences(data_train, voc_size) #relevance_mining(args, data_train, voc_size)
    med_adj = med_rel(args, data_train, voc_size)

    disease_frequencies = ehr_adj[:voc_size[0]]#, axis=1)
    procedure_frequencies = ehr_adj[voc_size[0]:]#, axis=1)
    # 2. 获取最低10%的频率
    if args.dataset == 'mimic-iii':
        disease_threshold = 37 #np.percentile(disease_frequencies, 25)
        procedure_threshold = 26 #np.percentile(procedure_frequencies, 25)
    else:
        disease_threshold = 188
        procedure_threshold = 7
    ...
    model = RCAMed(args, rare_diseases, rare_procedure, causal_graph, voc_size, ddi_adj, molecule_para, substruct_para, substruct_num=ddi_mask_H.shape[1])
    ...
    optimizer = Optimizer(model.parameters(), lr=args.lr, weight_decay=args.weight_decay, betas=(0.9, 0.994), eps=1e-6)  #0.995
