...
class COGNet(nn.Module):
    ...
    def __init__(self, args, voc_size, ehr_adj, ddi_adj, ddi_mask_H):
     ...
     #           nn.LayerNorm(emb_dim*3),
        ...
        self.pre_pat = nn.Linear(emb_dim*3, 3)
    ...
    def pre_visit(self,  batch_visits, max_len=20):
        device = self.device
        # batch维度以及seq维度上并行计算（现在不考虑时间序列信息），每一个medication序列仍然按顺序预测
        # batch_size, max_visit_num, max_med_num = medications.size()
        repr = []
        for adm in batch_visits:
            ...
            ############################ 数据预处理 #########################
            # 1. 对当前的disease与procedure进行编码
            input_disease_embdding = self.diag_embedding(torch.LongTensor(diseases).unsqueeze(dim=1).to(self.device))#.view(batch_size * max_visit_num, max_diag_num, self.emb_dim)      # [batch, seq, max_diag_num, emb]
            input_proc_embedding = self.proc_embedding(torch.LongTensor(procedures).unsqueeze(dim=1).to(self.device))#.view(batch_size * max_visit_num, max_proc_num, self.emb_dim)      # [batch, seq, max_proc_num, emb]
            
            # disease_embdding = self.conv_diag(diseases)
            
            input_disease_embdding = self.diagnoses_encoder(input_disease_embdding.permute(1,0,2))#.view(batch_size, max_visit_num, max_diag_num, self.emb_dim)
            input_proc_embedding = self.procedure_encoder(input_proc_embedding.permute(1,0,2))#, src_mask=p_enc_mask_matrix).view(batch_size, max_visit_num, max_proc_num, self.emb_dim)
            ...
            out_embedding = torch.concat([visit_diag_embedding, visit_proc_embedding], dim=1)
            repr.append(out_embedding)
        repr = torch.stack(repr)
        output = self.cls_pre(repr.squeeze(1))

        return output


    def pretrain(self, diseases, procedures, medications, d_mask_matrix, p_mask_matrix, m_mask_matrix, max_len=20):
        device = self.device
        # batch维度以及seq维度上并行计算（现在不考虑时间序列信息），每一个medication序列仍然按顺序预测
        batch_size, max_visit_num, max_med_num = medications.size()
        max_diag_num = diseases.size()[2]
        ...
        visit_diag_embedding = torch.sum(visit_diag_embedding, dim=1)
        visit_proc_embedding = torch.sum(visit_proc_embedding, dim=1)
        # encoded_medication = torch.sum(encoded_medication, dim=1)

        return visit_diag_embedding, visit_proc_embedding
