...
class AIModel(nn.Module):
    ...
    def used_med_learning(self, used_medications):
        ...
        results = []
        # 对于每一个visit的used_medications进行RNN编码
        for i in range(batch_size):
            if len(used_medications[i]) != 0:
                used_med_emb = self.used_med_embedding(torch.tensor(used_medications[i]).to(self.device)).sum(
    ...
    def forward(self, diagnose, procedures, used_medications, used_diag, used_proc, substruct_data, d_mask_matrix,
                p_mask_matrix):
        ...
        # print(diag_emb.shape,' ',proc_emb.shape)

        # mask_matrix用来mask掉注意力不需要关注的；repeat：匹配多头注意力机制；shape：[batch_size * nhead, max_diag_num, max_diag_num]
        ...
        diagnose_embdding = self.diagnoses_encoder(diag_emb,
                                                   src_mask=d_enc_mask_matrix)  # 通过Transformer编码后，shape不变，和diag_emb一样[batch_size, max_diag_num, emb]
        procedure_embedding = self.procedure_encoder(proc_emb,
                                                     src_mask=p_enc_mask_matrix)  # 通过Transformer编码后，shape不变，和diag_emb一样[batch_size, max_proc_num, emb]
        diag_enc = torch.sum(diagnose_embdding, dim=1)
        proc_enc = torch.sum(procedure_embedding, dim=1)

        # 将某visit的diag和proc编码合起来组成该visit的编码
        ...
        embedding1 = \
        self.crosssAtte1(substruct_information.unsqueeze(dim=0).repeat(batch_size, 1, 1), diagnose_embdding,
                         diagnose_embdding)[0]  # torch.Size([batch_size, 491, 112])
        embedding2 = \
        self.crosssAtte2(substruct_information.unsqueeze(dim=0).repeat(batch_size, 1, 1), procedure_embedding,
                         procedure_embedding)[0]
        diag_enc_cross = torch.sum(embedding1, dim=1)
        proc_enc_cross = torch.sum(embedding2, dim=1)
        # 将diag与med交叉注意力融合得到的emb 与 proc与med交叉注意力融合得到的emb 组合成新的visit编码
        visit_enc_cross = torch.cat([diag_enc_cross, proc_enc_cross], dim=-1)  # [batch_size, 2*emb]
        ######新添加#######对visit_enc_cross进行自注意力，使得diag和proc信息进一步交互
        embedding3 = self.crosssAtte3(visit_enc_cross.unsqueeze(dim=1), visit_enc_cross.unsqueeze(dim=1),
                                      visit_enc_cross.unsqueeze(dim=1))[0]  # [batch_size, 1, 2*emb]
        ...
        for i in range(batch_size):
            if len(used_diag[i]) != 0:
                ...
                # 提取最后一次visit的数据
                # used_diag [[], [], [], [], [], [], [], [[15, 146, 130, 122, 474, 700, 239, 832, 500, 442]], [[762, 767, 758, 759, 8, 404, 1911, 1837, 773]], [], [], [], [], [], [], []]
                # used_diag[i] [[762, 767, 758, 759, 8, 404, 1911, 1837, 773]]
                # usedDiag [762, 767, 758, 759, 8, 404, 1911, 1837, 773]
                usedDiag = used_diag[i][j]  # len:15
                usedProc = used_proc[i][j]  # len:6
                usedMed = used_medications[i][j]  # len:112（因为已编码）

                # 编码t-1次的diag,med,proc
                usedDiagEmbedding = self.diagnoses_encoder(
                    self.diag_embedding(torch.tensor(usedDiag).to(self.device)).unsqueeze(
                        dim=0))  # 经过Transformer torch.Size 不变 ([1, 15, 112])
                usedProcEmbedding = self.procedure_encoder(
                    self.proc_embedding(torch.tensor(usedProc).to(self.device)).unsqueeze(
                        dim=0))  # torch.Size([1, 6, 112])
                ...
                usedMedembedding = self.used_med_embedding(torch.tensor(usedMed).to(self.device)).sum(dim=1).unsqueeze(
                    dim=0)  # torch.Size([1, 112])

                # 交叉注意力
                last_embed1 = self.crosssAtte4(usedMedembedding.unsqueeze(dim=0), usedDiagEmbedding, usedDiagEmbedding)[
                    0]  # shape:[1, 1, emb]
                last_embed2 = self.crosssAtte5(usedMedembedding.unsqueeze(dim=0), usedProcEmbedding, usedProcEmbedding)[
                    0]  # [1, 1, emb]
                ...
                used_embedding3 = self.crosssAtte6(used_visit_enc.unsqueeze(dim=1), used_visit_enc.unsqueeze(dim=1),
                                                   used_visit_enc.unsqueeze(dim=1))[0]  # [1, 1, 3*emb]
        ...
        # 通过矩阵乘法计算每个药物组合的相互作用得分
        sigmoid_output_ddi = torch.matmul(sigmoid_output.unsqueeze(1).transpose(-1, -2),
                                          sigmoid_output.unsqueeze(1))  # [batch_size,vocab_size[2],vocab_size[2]]
        kg_ddi = torch.from_numpy(self.ddi_adj).to(sigmoid_output.device).unsqueeze(0).repeat(batch_size, 1,
                                                                                              1)  # [batch_size,vocab_size[2],vocab_size[2]]
