...
class CausaltyReview(nn.Module):
    ...
    def forward(self, pre_prob, diags, procs):
        ...
        # 根据条件更新 reviewed_prob
        low_condition = (max_cdm < self.c1_low_limit[0]) & (max_cpm < self.c1_low_limit[1])
        high_condition = (max_cdm > self.c1_high_limit[0]) | (max_cpm > self.c1_high_limit[1])

        reviewed_prob[0, low_condition] -= self.c1_minus_weight
        reviewed_prob[0, high_condition] += self.c1_plus_weight
...
class MoleRecModel(torch.nn.Module):
    def __init__(
        self, causal_graph, global_para, substruct_para, emb_dim, voc_size,
