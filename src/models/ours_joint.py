...
class PatientEncoder(nn.Module): # 把patient_encoder抽象出来，方便后续的模型调用 
    def __init__(self, args, rare_diseases, rare_procedure, voc_size, molecule_para, substruct_para, substruct_num):
    ...
    def patient_encoder_unified(self, batch_visits, substruct_data, mol_data, average_projection, ddi_mask_H):
        ...
        for adm in batch_visits:
            ...
            repr_d.append(torch.sum(visit_representation[1:len(diseases)], dim=0))
            repr_p.append(torch.sum(visit_representation[1+len(diseases):], dim=0))
            #visit_representation = visit_representation[0]
            drug_embed = self.drug_emb(visit_representation[0], substruct_embeddings, global_embeddings, ddi_mask_H.to(self.device)) 

        
            visit_representation = visit_representation.permute(1,0,2)#torch.reshape(visit_representation, (1,1,-1)) 
            drug_embed = drug_embed.expand(1, visit_representation.size(1), -1)
...
class RAREMed(PatientEncoder):
    def __init__(self, args, rare_diseases, rare_procedure, causal_graph, voc_size, ddi_adj, molecule_para, substruct_para, substruct_num):
        super(RAREMed, self).__init__(args, rare_diseases, rare_procedure, voc_size, molecule_para, substruct_para, substruct_num)
        ...
        self.cls_mask = LabelAttention(self.emb_dim+128, 1024, self.voc_size[0]+self.voc_size[1])#nn.Linear(self.emb_dim+128, self.voc_size[0]+self.voc_size[1])
        self.cls_med = LabelAttention(self.emb_dim+128, 1024, self.voc_size[2])#nn.Linear(self.emb_dim+128, self.voc_size[0]+self.voc_size[1])
        ...
        self.cls_nsp =  LabelAttention(self.emb_dim+128, 1024, 4)#nn.Linear(self.emb_dim+128, 1)

        self.cls_final =  LabelAttention(self.emb_dim+128, 1024, self.voc_size[2])#nn.Linear(self.emb_dim+128, self.voc_size[2])
    ...
    def forward_finetune(self, input, substruct_data, mol_data, average_projection, ddi_mask_H):
        ...
        patient_repr = torch.cat(patient_repr, dim=0)  # (B,1,dim)
    ...
    def forward(self, input, substruct_data, mol_data, ddi_mask_H, tensor_ddi_adj, average_projection, mode='fine-tune'):
        ...
        if mode == 'fine-tune':
            ...
            result = []
            result_med = []
            for i in patient_repr:
                result.append(self.cls_mask(i))             # (B, 1)
                result_med.append(self.cls_med(i))
            result = torch.cat(result, dim=0)
            result_med = torch.cat(result_med, dim=0)
            result = result.squeeze(dim=1)   
            result_med = result_med.squeeze(dim=1)  # (B,)
            return result, result_med
            ...
            result = []
            for i in patient_repr:
                result.append(self.cls_nsp(i))             # (B, 1)
            result = torch.cat(result, dim=0)
            result = result.squeeze(dim=1)                  # (B,)
            # logit = F.sigmoid(result) # logistic regression
            return result, repr_d, repr_p
        
        elif mode == 'joint':
            patient_repr, repr_d, repr_p = self.patient_encoder(input, substruct_data, mol_data, average_projection, ddi_mask_H)      # (B, dim)
            result = []
            result_med = []
            for i,p in enumerate(patient_repr):
                if i == 0 or i == 3:
                    result.append(self.cls_mask(p))             # (B, 1)
                    result_med.append(self.cls_med(p))
                result_nsp.append(self.cls_nsp(p))
            result_med = torch.cat(result_med, dim=0)
            result_nsp = torch.cat(result_nsp, dim=0)
            result = result.squeeze(dim=1)   # (2*B,) for mask
            result_med = result_med.squeeze(dim=1)  # (2*B,) for med
            result_nsp = result_nsp.squeeze(dim=1)  # (4*B,) for nsp
            return result, result_med, result_nsp, repr_d, repr_p
