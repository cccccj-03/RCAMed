...
from utils.layers import SelfAttend
from utils.layers import GraphConvolution

from utils.modules import *
from utils.block_recurrent_transformer.transformer import BlockRecurrentAttention
...
class SHAPE(nn.Module):
    ...
    def __init__(self, voc_size, ehr_adj, ddi_adj, ddi_mask_H, emb_dim=128, device=torch.device('cpu:0'), num_inds=32,
                 dim_hidden=128, num_heads=2, ln=False, isab_num=2, kgloss_alpha=0.001):
        ...
        # 这里采用的set-encoder模块
        # ISAB:两层attention，第一层诱导点为Q，第二层输入集合为Q
    ...
    def set_encoder(self, input, attn_mask, rep=2):# 两层ISAB
    ...
    def forward(self, diseases, procedures, medications, d_mask_matrix, p_mask_matrix, m_mask_matrix, seq_length,
        ...
        new_medication = torch.cat([new_medication, medications[:, :-1, :]],
                                   dim=1)  # new_medication.shape=[b,max_seq_len, max_med_num]
        ...
        # # 3.1 diagnosis 的set-encoders(建模输入集合内部的元素交互)
        d_enc_mask_matrix = d_mask_matrix.view(batch_size * max_seq_length, max_diag_num).unsqueeze(1).repeat(1,
                                                                                                              self.nhead,
                                                                                                              1)  # [batch_size*visit_num, nhead, diag_len]
        ...
        p_enc_mask_matrix = p_mask_matrix.view(batch_size * max_seq_length, max_proc_num).unsqueeze(1).repeat(1,
                                                                                                              self.nhead,
                                                                                                              1)  # [batch_size*visit_num, nhead, proc_len]
        ...
        m_enc_mask_matrix = new_m_mask.view(batch_size * max_seq_length, max_med_num).unsqueeze(1).repeat(1, self.nhead,
                                                                                                          1)  # [batch_size*visit_len, nhead, med_len]
        ...
        # 4.将三种code分别aggregate，转化为visit-level(将一次visit中的多个医疗code整合起来)
        ...
        sigmoid_output_ddi = torch.matmul(sigmoid_output.unsqueeze(2).transpose(-1, -2), sigmoid_output.unsqueeze(
            2))  # [batch_size, max_seq_length, vocab_size, vocab_size]
        kg_ddi = torch.from_numpy(self.ddi_adj).to(sigmoid_output.device).unsqueeze(0).unsqueeze(0).repeat(batch_size,
                                                                                                           max_seq_length,
                                                                                                           1,
                                                                                                           1)  # [batch_size, max_seq_length, vocab_siz, vocab_size]
