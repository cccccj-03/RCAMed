...
class mixprop(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        h = x
        out = [h]
        #a = adj / d.view(-1, 1) + 1e-7
        for i in range(self.gdep):
            h = self.alpha * x + (1 - self.alpha) * self.nconv(h, adj)
            h = self.ln(h.permute(0,2,1)).permute(0,2,1)
            out.append(h)

        ho = torch.cat(out, dim=1)
        ho = self.mlp(ho)

        return ho.permute(2, 0, 1)
...
class PatientEncoder(nn.Module): # 把patient_encoder抽象出来，方便后续的模型调用 
    def __init__(self, args, ehr_adj, rare_diseases, rare_procedure, voc_size, molecule_para, substruct_para, substruct_num):
        ...
        self.tensor_ehr_adj = torch.FloatTensor(ehr_adj).to(self.device)
        ...
        self.graph_fc = mixprop(self.emb_dim, self.emb_dim, gdep=3, dropout=0.3, alpha=0)
    ...
    def patient_encoder_unified(self, batch_visits, substruct_data, mol_data, average_projection, ddi_mask_H):
        ...
        for adm in batch_visits:
            ...
            degree = submatrix.sum(dim=1)
            submatrix.diagonal().copy_(degree)
            # D_inv_sqrt = torch.diag(torch.pow(degree + 1e-7, -0.5))
            # submatrix = D_inv_sqrt @ submatrix @ D_inv_sqrt
            ...
            graph_embedding = torch.concat([disease_embedding, procedure_embedding], dim=0)
            graph_embedding = self.graph_fc(graph_embedding, submatrix)
            # graph_d = graph_embedding[:len(diseases)]
            # graph_p = graph_embedding[len(diseases):]
            graph_d = F.sigmoid(disease_embedding @ graph_embedding[:len(diseases)].transpose(1,2)) @ disease_embedding
            graph_p = F.sigmoid(procedure_embedding @ graph_embedding[len(diseases):].transpose(1,2)) @ procedure_embedding
            disease_embedding = disease_embedding + graph_d
            procedure_embedding = procedure_embedding + graph_p
...
class RAREMed(PatientEncoder):
    def __init__(self, args, ehr, rare_diseases, rare_procedure, causal_graph, voc_size, ddi_adj, molecule_para, substruct_para, substruct_num):
        super(RAREMed, self).__init__(args, ehr, rare_diseases, rare_procedure, voc_size, molecule_para, substruct_para, substruct_num)
