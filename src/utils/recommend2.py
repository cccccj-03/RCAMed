...
from utils.data_loader import mimic_data, pad_num_replace
...
from utils.utils_shape import llprint, sequence_metric, sequence_metric_v2, sequence_output_process, ddi_rate_score, get_n_params, \
    print_result
...
def eval_recommend_batch(model, batch_data, device, TOKENS, args):
    ...
    output_logits = model(diseases, procedures, medications, d_mask_matrix, p_mask_matrix, m_mask_matrix, seq_length,
                          dec_disease, stay_disease, dec_disease_mask, stay_disease_mask,
                          dec_proc, stay_proc, dec_proc_mask, stay_proc_mask)
    if args.model_name == 'SetEncoder_RTB_adLR_addDDIinlast' or args.model_name == 'SetEncoder_RTB_adLR_addDDIinlastAndEncode' or args.model_name == 'SetEncoder_RTB_adLR_addEHRinEncoder_addDDIinlast' or args.model_name == 'SetEncoder_RTB_adLR_addDDIinlast_noSet' or args.model_name == 'SetEncoder_RTB_adLR_addDDIinlast_noRTB' or args.model_name == 'SetEncoder_RTB_adLR_addDDIinlast_noLR' or args.model_name == 'SetEncoder_RTB_adLR_addDDIinlast_noSetUseSelfAtt':
...
def test_recommend_batch(model, batch_data, device, TOKENS, ddi_adj, args):
    ...
    # input_disease_embdding = input_disease_embdding.repeat_interleave(args.beam_size, dim=0) # [batch_size, visit_len, code_len, h]
...
# evaluate
def eval(model, eval_dataloader, voc_size, device, TOKENS, args, ddi_adj_path, rec_results_path=None):
    model.eval()
    END_TOKEN, DIAG_PAD_TOKEN, PROC_PAD_TOKEN, MED_PAD_TOKEN, SOS_TOKEN = TOKENS
    ...
    for idx, data in enumerate(eval_dataloader):
        ...
        # 改
        output_tensor = output_logits[0]
        predictions = torch.sigmoid(output_tensor).cpu().detach().numpy()
        ...
        # +
        max_len = max(len(seq) for seq in y_pred_label)
        y_pred_label = [np.array(seq).flatten().tolist() if isinstance(seq, (list, np.ndarray)) else seq for seq in
                        y_pred_label]
        y_pred_label = [seq + [0] * (max_len - len(seq)) for seq in y_pred_label]
...
def test(model, test_dataloader, diag_voc, pro_voc, med_voc, voc_size, epoch, device, TOKENS, ddi_adj, args):
    ...
    saved_path = os.path.dirname(args.resume_path)
    pickle.dump(all_pred_list, open(saved_path + '/out_list.pkl', 'wb'))
    pickle.dump(all_label_list, open(saved_path + '/out_list_gt.pkl', 'wb'))
    pickle.dump(record_ja, open(saved_path + '/record_ja.pkl', 'wb'))
    pickle.dump(record_prauc, open(saved_path + '/record_prauc.pkl', 'wb'))
    pickle.dump(record_p, open(saved_path + '/record_p.pkl', 'wb'))
    pickle.dump(record_r, open(saved_path + '/record_r.pkl', 'wb'))
    pickle.dump(record_f1, open(saved_path + '/record_f1.pkl', 'wb'))
    # saved_path = "/home/lsc/model/lsc_code/useGeneration2Drug/generate2Drug/set_gMed/fine-tuning/saved/Set_GMed_reduceSetDecoder/total_test_result"
    # pickle.dump(all_pred_list, open(saved_path + '/out_list.pkl', 'wb'))
    # pickle.dump(all_label_list, open(saved_path + '/out_list_gt.pkl', 'wb'))
    # pickle.dump(record_ja, open(saved_path+'/record_ja.pkl', 'wb'))
    # pickle.dump(record_prauc, open(saved_path+'/record_prauc.pkl', 'wb'))
